...
export async function runPipeline(): Promise<PipelineResult> {
  // The daily prayer is generated by the Claude scheduled task (free, runs at 03:00 UTC,
  // posts via the API and goes through the same editorial approval gate). This built-in
  // pipeline is kept only as a manual fallback: it refuses to run unless explicitly
  // enabled, so the two systems can never double-post.
  if (process.env.DAILY_PIPELINE_ENABLED !== 'true') {
    console.log('[DAILY-PRAYER] Skipped: pipeline disabled (set DAILY_PIPELINE_ENABLED=true to use as fallback). The Claude scheduled task is the daily generator.');
